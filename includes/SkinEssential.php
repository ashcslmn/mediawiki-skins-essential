...
class SkinEssential extends SkinMustache {
	...
	private function buildSearchProps() : array {
		$config = $this->getConfig();
		$query = empty($this->getRequest()->getQueryValues()['search']) ? '' : $this->getRequest()->getQueryValues()['search'] ;
		$props = [
			...
			'html-input' => $this->makeSearchInput( [ 
					'id' => 'searchInput', 
					"class" => "md:w-full h-10 md:h-14 px-2 w-3/4 text-black-leni focus:ring focus:ring-blue-leni focus:ring-opacity-50", 
					'placeholder' => 'Ano ang hinahanap mo?',
					'value' => $query
				]
			),
